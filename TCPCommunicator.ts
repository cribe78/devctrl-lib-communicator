...
import {Control, ControlUpdateData, IEndpointStatus, IndexedDataSet} from "@devctrl/common";
...
export class TCPCommunicator extends EndpointCommunicator {
  ...
  openConnection(attempt: number = 0) {
    if (this.closingConnection) {
    ...
    let elapsed = Date.now() - this.lastConnectAttemptTime;
    if ( elapsed < this.backoffTime) {
      if (! this.lastConnectTimeout) {
          this.connectionAttempt++;
          let attemptNo = this.connectionAttempt;
          this.log(`rate limiting reconnection, backing off for ${this.backoffTime}ms before attempt ${attemptNo}`);
        this.lastConnectTimeout = setTimeout(() => {
          this.openConnection(attemptNo);
        }, this.backoffTime);
        return;
      }
      else {
          this.log(`reconnection attempt already scheduled, will wait for that`);
          return;
      }
    } else {
      this.log(`backoff time exceeded (${elapsed}), proceeding with open connection attempt ${attempt}`);
    }
    ...
    this.log(`opening TCP connection to ${connectOpts.host}:${connectOpts.port}, attempt ${attempt}`, EndpointCommunicator.LOG_CONNECTION);
  ...
  writeToSocket(val: string) {
    ...
    this.commsLogger.log(bufferToSend, DataDirection.out);
